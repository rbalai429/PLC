...
namespace HPPlc.Controllers
{
	public class NotificationController : SurfaceController
	{
		...
		public void SendWhatsappNotification_VK()
		{
			...
			var notificationDatas = new dbNotificationAccess().GetNotificationData("whatsapp");

			if (notificationDatas != null)
			{
				foreach (NotificationData item in notificationDatas)
				{
					SendWhatsAppNotification(whatsappNotificationEvents, item);
				}
			}
		}
		public void SendWhatsAppNotification
			(IEnumerable<WhatsappNotifications> whatsappNotificationEvents, NotificationData notificationData)
		{
			try
			{
				var whatsappNotificationEvent = whatsappNotificationEvents?
						.Select(x => x.WhatsappNotificationEvents.FirstOrDefault(y => y.NumberOfDays == notificationData.NoOfDays.ToString().Trim()))?
						.FirstOrDefault();

				if (whatsappNotificationEvent != null)
				{
					WhatsAppNotificationNew(whatsappNotificationEvent, notificationData);
				}
			}
			catch (Exception ex)
			{
		...
		//Added by v
		public string WhatsAppNotificationNew(WhatsappNotificationEvent notificationItem, NotificationData notificationData)
		{
			GetStatus responce = new GetStatus();

			// TODO: get event code from notification item
			var eventCode = notificationItem.EventCode;

			// TODO: get notification message from this event code

			//RI: telling us plan expiring within (15,10,5,1) days for paid user
			//DLE: telling us all free worksheets are downloaded for both user (3,12,30)
			//D:  telling us free worksheets are remaining for download for both user 
			...
			try
			{
				...
				if (notificationItem != null)
				{
					...
					if (notificationItem.BannerUrl != null)
						whatsappBannerr = notificationItem?.BannerUrl;
					else
					{
						whatsappBannerr = Umbraco.ContentAtRoot()?.Where(x => x.ContentType.Alias == "home")?.FirstOrDefault()?.DescendantsOrSelf()?
													.Where(x => x.ContentType.Alias == "worksheetNode")?.OfType<WorksheetNode>()?.FirstOrDefault()?.FreeUserLastWeekBanner?.Url();
					...
					if (!String.IsNullOrWhiteSpace(notificationItem.NotificationUrl))
						redirectURL = notificationItem?.NotificationUrl;
					...
					var noOfDays = notificationItem.NumberOfDays;
					if (noOfDays == "3" || noOfDays == "12" || noOfDays == "30")
					{
						if (notificationData.NotificationTypeCode == "DLE")
						{
							//TODO: send Download limit exhausted
						}
						else if (notificationData.NotificationTypeCode == "D")
						{
							//TODO: send Download remaining
						}
					}
					else
					{
						//TODO: send common message
					}

					if (notificationData.NotificationTypeCode == "RI")
					{
						//TODO: send plan expiary message
					}

					notificationLog.Add(SendNotificationNew(notificationItem, notificationData, whatsappBannerr, redirectURL, "", 0, "", notificationItem.EventCode, "I"));
				}
			}
			catch (Exception ex)
			{
		...
		private NotificationLog SendNotificationNew(WhatsappNotificationEvent NotifData,
			NotificationData notifypersonaldata, string BannerImage, string pdfUrl,
			string TopicName, int worksheetId, string SubjectName = "",
			string notificationtype = "", string TypeOfNotif = "")
		{
			string mobile = String.Empty;
			string name = String.Empty;
			...
			if (NotifData != null)
			{
				if (!String.IsNullOrWhiteSpace(notifypersonaldata.MobileNo)
					&& !String.IsNullOrWhiteSpace(NotifData?.EventCode))
				{
					mobile = clsCommon.Decrypt(notifypersonaldata.MobileNo);
					if (!String.IsNullOrWhiteSpace(mobile) && (mobile.Substring(0, 3) == "+91" || mobile.Length == 10))
						mobile = "91" + mobile;
					...
					else
					{
						...
						//subject
						if ( !String.IsNullOrWhiteSpace(NotifData?.NotificationSubject))
							lst.Subjects = NotifData.NotificationSubject;
					...
					WhatsAppHelper whatsAppHelper = new WhatsAppHelper();
					IRestResponse response = whatsAppHelper.CreateMessage(mobile, NotifData.EventCode, lst);

					if (response != null)
					{
						notificationLog.UserId = notifypersonaldata.UserId;
						notificationLog.WeekId = notifypersonaldata.NoOfWeek;
						notificationLog.NotificationName = NotifData.EventCode;
						notificationLog.NotificationStatus = 1;
						notificationLog.NotificationJson = response.Content;
						notificationLog.SendStatus = response.StatusCode.ToString();
						notificationLog.MobileNo = notifypersonaldata.MobileNo;
						notificationLog.AgeGroup = notifypersonaldata.MinAgegroup;
						notificationLog.Subject = NotifData.NotificationSubject;
						notificationLog.NotificationType = notificationtype;
						notificationLog.BannerUrl = BannerImage;
						notificationLog.PdfUrl = pdfUrl;
						notificationLog.TypeOfNotif = TypeOfNotif;
