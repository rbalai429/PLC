...
namespace HPPlc.Controllers
{
    public class NotificationController : SurfaceController
    {
            ...
            if (notificationDatas != null)
            {
                foreach (NotificationData item in notificationDatas)
                {
                    var whatsappNotificationEvent = whatsappNotificationEvents?
                        .Select(x => x.WhatsappNotificationEvents
                        .FirstOrDefault(y => y.NumberOfDays == item.NoOfDays.ToString().Trim()
                        && y.NotificationType.ToLower() == item.NotificationTypeCode.ToLower()))?
                        .FirstOrDefault();

                    if (whatsappNotificationEvent != null)
                    {
                        WhatsAppNotificationNew(whatsappNotificationEvent, item);
                    }
                }
            }
        }
        ...
        //Added by v
        private string WhatsAppNotificationNew(WhatsappNotificationEvent notificationItem, NotificationData notificationData)
        {
            GetStatus responce = new GetStatus();
            ...
            try
            {
                var notificationLog = new List<NotificationLog>();
                ...
                if (notificationItem != null)
                {
                    ...
                    {
                        whatsappBannerr = Umbraco.ContentAtRoot()?.Where(x => x.ContentType.Alias == "home")?
                            .FirstOrDefault()?
                            .DescendantsOrSelf()?
                            .Where(x => x.ContentType.Alias == "worksheetNode")?
                            .OfType<WorksheetNode>()?
                            .FirstOrDefault()?.FreeUserLastWeekBanner?.Url();
                    }
                    ...
                    notificationLog.Add(SendWhatsappNotificationForWorksheet(notificationItem, notificationData, whatsappBannerr, redirectURL, "", 0, notificationItem.EventCode));
                }
            }
            catch (Exception ex)
            {
        ...
        private NotificationLog SendWhatsappNotificationForWorksheet(WhatsappNotificationEvent NotifData,
            NotificationData notifypersonaldata, string bannerImage, string pdfUrl,
            string topicName, int worksheetId,
