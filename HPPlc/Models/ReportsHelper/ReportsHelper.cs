	...
	public class ReportsHelper
	{
		...
		public Responce GetAllRegistrationList(FilterInput filter)
		{
			...
			try
			{
				...
				List<SetParameters> sp = new List<SetParameters>()
				{
					...
					new SetParameters{ ParameterName = "@StartDate", Value = Convert.ToDateTime(filter.StartDate).ToString("yyyy-MM-dd") },
					new SetParameters{ ParameterName = "@EndDate", Value = Convert.ToDateTime(filter.EndDate).ToString("yyyy-MM-dd") }
				};
		...
		public List<SubscriptionsItemForCouponCode> GetSubscriptionList()
		{
			...
			if (Subscription != null && Subscription.Count() > 0)
			{
				...
				foreach (var item in Subscription.Where(x => x.DiscountAmount > 0))
				{
		...
		public Responce CreateEditCouponCode(CreateCouponCodeModel input)
		{
			...
			{
				dbProxy _db = new dbProxy();
				if (input.IsAppliedForMultipleUserType != null)
					input.IsAppliedForMultipleUserTypeValue = (input.IsAppliedForMultipleUserType.HasValue && input.IsAppliedForMultipleUserType.Value) ? 1 : 2;

				if (input.IsAppliedForSubscription != null)
					input.IsAppliedForSubscriptionValue = (input.IsAppliedForSubscription.HasValue && input.IsAppliedForSubscription.Value) ? 1 : 2;

				if (input.IsCouponAppliedForAgeGroup != null)
					input.IsCouponAppliedForAgeGroupValue = (input.IsCouponAppliedForAgeGroup.HasValue && input.IsCouponAppliedForAgeGroup.Value) ? 1 : 2;
				...
				if (input.CouponCodeId == 0)
				{
					List<SetParameters> sp = new List<SetParameters>()
			{
				new SetParameters{ ParameterName = "@TransactionCode", Value = input.TransactionId!=null?input.TransactionId:"" },
				new SetParameters{ ParameterName = "@CouponCodePrefix", Value = input.CouponCodePrefix.ToString() },
				new SetParameters{ ParameterName = "@CouponLimit", Value = input.NoOfCouponCode.ToString() },
				new SetParameters { ParameterName = "@CouponType", Value = input.CouponType },
				new SetParameters { ParameterName = "@UserType", Value = input.SelectedUserType.ToString() },
				new SetParameters { ParameterName = "@CouponUsagesType", Value = input.CouponUsagesType.ToString() },
				...
				new SetParameters { ParameterName = "@CouponSource", Value = input.CouponSource.ToString()},
				new SetParameters { ParameterName = "@DiscountType", Value = input.DiscountType.ToString() },
				new SetParameters { ParameterName = "@DiscountValue", Value = input.DiscountValue.ToString()},
				new SetParameters { ParameterName = "@IsAppliedForSubscription", Value = input.IsAppliedForSubscription.HasValue && input.IsAppliedForSubscription.Value?"1":"2"},
				new SetParameters { ParameterName = "@SubscriptionValue", Value = input.SelectedSubscription },
				new SetParameters { ParameterName = "@IsCouponAppliedForAgeGroup", Value = input.IsCouponAppliedForAgeGroup.HasValue && input.IsCouponAppliedForAgeGroup.Value?"1":"2"},
				new SetParameters { ParameterName = "@AgeGroupValue", Value = input.SelectedAgeGroup },
				new SetParameters { ParameterName = "@IsCouponAppliedForDomain", Value = input.IsAppliedForDomain.HasValue && input.IsAppliedForDomain.Value?"1":"2"},
				new SetParameters { ParameterName = "@DomainValue", Value = input.DomainId!=null?input.DomainId:""}
			};

					insertStatus = _db.StoreData("USP_CouponCodeEntry", sp);
					...
					List<SetParameters> sp = new List<SetParameters>()
			{
				...
				new SetParameters { ParameterName = "@UserId", Value = input.UserId.ToString() },
					...
					insertStatus = _db.StoreData("EditCouponCodeLog", sp);
					if (insertStatus != null && insertStatus.returnStatus == "Success")
					{
						responce.Result = insertStatus.returnValue;
						responce.Message = insertStatus.returnMessage;
		...
		public Responce CreateEditCouponCodeWithDT(List<CreateCouponCodeModel> CouponCodeList)
		{
			Responce responce = new Responce();
			try
			{
				DataTable CouponCodeDT = ToDataTable<CreateCouponCodeModel>(CouponCodeList);
				dbProxy _db = new dbProxy();
				GetStatus insertStatus = new GetStatus();
				DataSet ds = new DataSet();
				ds.Tables.Add(CouponCodeDT);
				string XmlParameter = ds.GetXml();
				...
				insertStatus = _db.StoreData("Insert_CouponCode_WithExcel", sp);
				if (insertStatus.returnStatus == "Success")
				{
					responce.StatusCode = HttpStatusCode.OK;
					responce.Message = insertStatus.returnStatus;
		...
		public bool SendEmailToAdminCouponCodeGenerated(MailContentModel mailContent, int NoOfCouponCode, string TransactionId)
		{
			bool result = true;
			SenderMailer mailer = new SenderMailer();
			if (mailContent != null)
			{
				string Body = "Hello Admin \n Coupon Code has been gererated. \n there are : " + NoOfCouponCode + " Coupon Code Created!\n Our Transaction Id: : " + TransactionId + "\n Gererated date:" + DateTime.Now.ToString("dddd, dd MMMM yyyy");
				mailer.SendMail(mailContent.EmailTo, mailContent.Subject, Body, mailContent.EmailCC, mailContent.EmailBcc);
			}
			return result;
		}
		...
		public Responce GetCouponCodeEditDetails(int CouponCodeId)
		{
			...
			try
			{
				...
				{
					CreateCouponCodeModel.TransactionId = EditCode.TransactionId;
		...
		public Responce InsertCouponCodeFromExcel(CreateCouponCodeModel input)
		{
			Responce responce = new Responce();
			dbProxy _db = new dbProxy();
			GetStatus insertStatus = new GetStatus();
				...
				List<SetParameters> sp = new List<SetParameters>()
			{
				...
				new SetParameters { ParameterName = "@IsCouponAppliedForDomain", Value = input.IsAppliedForDomain.HasValue && input.IsAppliedForDomain.Value?"1":"2"},
				new SetParameters { ParameterName = "@DomainValue", Value = input.DomainId!=null?input.DomainId:""},
				new SetParameters { ParameterName = "@FileType", Value = input.filetype},
				new SetParameters { ParameterName = "@XMLFile", Value = input.fileupload}
			};

				insertStatus = _db.StoreData("USP_CouponCodeEntryExcel", sp);
				if (insertStatus != null && insertStatus.returnStatus == "Success")
				{
					responce.Result = insertStatus.returnValue;
					responce.Message = insertStatus.returnMessage;
			...
			catch (Exception ex)
			{
				...
				error.PageName = "ReportsHelper";
				error.MethodName = "InsertCouponCodeFromExcel";
		...
		public Responce GetAllNotificationList(FilterInput filter)
		{
			Responce responce = new Responce();
			try
			{
				...
				List = _db.GetDataMultiple("GET_ALL_NEW_REPORTS", List, sp);
				//if (List != null && List.Count > 0)
				//{
				//	for (int i = 0; i < List.Count; i++)
				//	{
		...
		#endregion
		#region User Report Download Data
		public Responce GetUserReportDownloadDataList(FilterInput filter)
		{
			...
			try
			{
				...
				//		item.UserName = clsCommon.Decrypt(item.UserName); // Decrypt The value 
				//}
		...
		public Responce GetDownloadReportDataList(FilterInput filter)
		{
			Responce responce = new Responce();
			try
			{
			...
			try
			{
				...
				if (List != null && List.Count > 0)
				{
					foreach (var item in List)
					{
						if (!String.IsNullOrEmpty(item.UserName))
							item.UserName = clsCommon.Decrypt(item.UserName); // Decrypt The value 
						if (!String.IsNullOrEmpty(item.RefToUserName))
							item.RefToUserName = clsCommon.Decrypt(item.RefToUserName);
					}
				}
		...
		public Responce InsertURLManipulationList(string oldUrl, string newUrl)
		{
			Responce responce = new Responce();
			try
			{
				...
				List<SetParameters> sp = new List<SetParameters>()
				{
					new SetParameters{ ParameterName = "@OldUrl", Value = oldUrl},
					new SetParameters{ ParameterName = "@NewUrl", Value =  newUrl}
				};
				List = _db.GetDataMultiple("INSERT_URL_MANIPULATION", List, sp);
        ...
        #endregion
        #region Worksheet Bulk upload
        public Responce Get_AgeGroupe_Language_Master(string vMasterType)
        {
            Responce responce = new Responce();
            try
            {
                ...
                List<SetParameters> sp = new List<SetParameters>()
                {
                    new SetParameters{ ParameterName = "@Qtype", Value = vMasterType}
                };
                if (vMasterType == "1")
                {
                    AgeGroupeList = _db.GetDataMultiple("GET_MASTERS_AGEGROUP_LANGUAGE", AgeGroupeList, sp);
                    responce.Result = AgeGroupeList;
                    responce.StatusCode = HttpStatusCode.OK;
                    return responce;
                }
                else if (vMasterType == "2")
                {
                    LanguageList = _db.GetDataMultiple("GET_MASTERS_AGEGROUP_LANGUAGE", LanguageList, sp);
                    responce.Result = LanguageList;
                    responce.StatusCode = HttpStatusCode.OK;
                    return responce;
                }
                else
                {
                    ProgramTypeList = _db.GetDataMultiple("GET_MASTERS_AGEGROUP_LANGUAGE", ProgramTypeList, sp);
                    responce.Result = ProgramTypeList;
                    responce.StatusCode = HttpStatusCode.OK;
                    return responce;
                }
            }
		...
		#endregion
		#region Send WhatsApp Notification Report 
		public Responce GetAllSendWhatsAppNotificationList(FilterInput filter)
		{
			Responce responce = new Responce();
			try
			{
		...
		public string InsertSendWhatsAppNotificationList(XDocument XmlDoc)
		{
			string responce = "";
			try
			{
				...
				{
						new SqlParameter("@XMLParameter",XmlDoc),
				};
				DataTable DT = _db.ExecStoredProc("PROC_INSERT_SendWhatsAppNotification", param, "DATATABLE");
				if (DT != null && DT.Rows.Count > 0)
		...
		public Responce InsertAgeGroupeAdvancedSearch(string ClassName, string SynonymsName)
		{
			Responce responce = new Responce();
			try
			{
				...
				List<SetParameters> sp = new List<SetParameters>()
				{
					new SetParameters{ ParameterName = "@ClassName", Value = ClassName},
					new SetParameters{ ParameterName = "@SynonymsName", Value =  SynonymsName}
				};
				List = _db.GetDataMultiple("INSERT_AGEGROUP_ADVANCESEARCH", List, sp);
		...
		public Responce GetAllAgeGroupeAdvancedList(FilterInput filter)
		{
			Responce responce = new Responce();
			try
			{
		...
		public Responce GetAllNoRecordFoundList(FilterInput filter)
		{
			...
			try
			{
				...
				List<SetParameters> sp = new List<SetParameters>()
				{
					new SetParameters{ ParameterName = "@QType", Value = "7"},
					new SetParameters{ ParameterName = "@StartDate", Value = Convert.ToDateTime(filter.StartDate).ToString("yyyy-MM-dd") },
					new SetParameters{ ParameterName = "@EndDate", Value = Convert.ToDateTime(filter.EndDate).ToString("yyyy-MM-dd") },
				};
				List = _db.GetDataMultiple("GET_ALL_NEW_REPORTS", List, sp);
				if (List != null && List.Count > 0)
				{
					foreach (var item in List)
					{
						if (!String.IsNullOrEmpty(item.Name))
							item.Name = clsCommon.Encrypt(item.Name);
						if (!String.IsNullOrEmpty(item.Email))
							item.Email = clsCommon.Encrypt(item.Email);
						if (!String.IsNullOrEmpty(item.Mobile))
							item.Mobile = clsCommon.Encrypt(item.Mobile);
					}
				}
				responce.Result = List;
				responce.StatusCode = HttpStatusCode.OK;
		...
		#region User Transaction Report 
				...
				List<SetParameters> sp = new List<SetParameters>()
		{
			...
			new SetParameters{ ParameterName = "@EndDate", Value = Convert.ToDateTime(filter.EndDate).ToString("yyyy-MM-dd") },
				...
				if (List != null && List.Count > 0)
				{
					...
					{
						if (!String.IsNullOrEmpty(item.UserName))
							item.UserName = clsCommon.Decrypt(item.UserName);
						...
						if (!String.IsNullOrEmpty(item.Emailid))
							item.Emailid = clsCommon.Decrypt(item.Emailid);
					}
				}
				responce.Result = List;
				responce.StatusCode = HttpStatusCode.OK;
