...
function PlayVideoUpdate(UniqueId, PlayDuration, PlayEnd,VideoSouce) {
	//alert('salik');
	$.ajax({
		type: 'POST',
		url: "/umbraco/Surface/Video/VideoTrackingUpdate",
		data: {
			'vrUniqueId': UniqueId, 'vrPlayDuration': PlayDuration, 'vrPlayEnd': PlayEnd, 'VideoSouce': VideoSouce },
		dataType: "json",
...
function PlayVideoView() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			console.log(isIOS);
			if (isIOS == true) {
				GetVimeoVideoUrl2(palyVid, 'hls', vid_executionTime, UniqueId)
			} else {
				GetVimeoVideoUrl2(palyVid, 'dash', vid_executionTime, UniqueId)
			}
	...
	var videoSource = $("#hdnQueryTypeBonus").val();
	if (videoSource != null && videoSource != undefined && videoSource == "bns") {
		//$(".card-btn-print").hide();
		$("#generalPrint").hide();
		$("#bonusdwnldPrint").show();
...
function GetVimeoVideoUrl2(videoId, type, vid_executionTime, UniqueId) {
	var VideoSource = $("#hdnQueryTypeBonus").val();
	var nextelemt = $('.active-video').parents('.slick-slide').next();
	var redircturl = nextelemt.find('a').attr('href');

	var videoName = $("#playVideoName").val();
	var videoSource = $("#hdnQueryTypeBonus").val();

	var hdndurationTillViewVideos = $("#hdndurationTillViewVideos").val();
	var hdnUserLoggedIn = $("#hdnUserLoggedIn").val();
	var hdnUserEligibleToWatchVideo = $("#hdnUserEligibleToWatchVideo").val();
	...
	$.ajax({
		...
		success: function (data) {
			...
			player.one('loadedmetadata', function () {
				...
				OpenVideo(videoName, duration);
				this.play();
			});

			player.currentTime(vid_executionTime);
			...
			player.on('play', function(data) {
				...
				PlayVideo(videoName, PlayTime);
			});

			player.on('pause', function (data) {
				var PlayDuration = player.currentTime();
				var PlayEnd = 0;

				PauseVideo(videoName)
				PlayVideoUpdate(UniqueId, PlayDuration, PlayEnd, VideoSource);
				
			});

			if (videoSource != null && videoSource != "" && videoSource != undefined && videoSource == "bns" && hdnUserEligibleToWatchVideo == false) {
				player.on('timeupdate', function (e) {
					
					if (player.currentTime() >= hdndurationTillViewVideos) {
						player.pause();
