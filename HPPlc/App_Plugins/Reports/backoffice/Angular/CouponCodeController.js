app.controller('CouponCodeController', function ($scope, $http, $timeout, Comman, $location, notificationsService, userService) {
	...
	$scope.AddNewCouponCodeExcel = function () {
		$scope.IsStartDateValid = false;
		$scope.model.showexcelCoupon = true;
		$scope.model.showing = true;
		$scope.ErrorForAgeRequired = '';
		$http.get(Comman.GetCreateCouponCodeDetailsUrl())
			.then(function (response) {
	...
	$scope.AddNewSerialCodeExcel = function () {
		$scope.IsStartDateValid = false;
		$scope.model.showexcelSerial = true;
		$scope.model.showing = true;
		$scope.ErrorForAgeRequired = '';
		$http.get(Comman.GetCreateCouponCodeDetailsUrl())
			.then(function (response) {
	...
	/*Save Excel for Coupon*/
	$scope.uploadCouponExcelFile = function (files) {
		$scope.files = files;
		var file = $scope.files.files[0];
		if (file != null && file != undefined) {
			var allowedExtensions =
			...
			if (!allowedExtensions.exec(file.name)) {
				...
				//};
				$http({
					url: Comman.CreateEditCouponCodeUrlExcelFileSave(),
					method: "POST",
					headers: { "Content-Type": undefined },
					transformRequest: function () {
						var formData = new FormData();
	...
	/*Save excel data for serial no*/
	$scope.SaveCouponCodeExcelCoupon = function (CreateCouponCodeFormExcelCoupon, CouponCode) {
		$scope.ErrorForAgeRequired = '';
		$scope.ErrorForSubscriptionRequired = '';
		$scope.ErrorForMultipleUserType = '';

		if (CreateCouponCodeFormExcelCoupon.$invalid || $scope.EndDateErrorMessage != '') {
			return;
		} else {
			var SelectedUserType = "";
			var SelectedSubscription = "";
			...
			var ExcelExistsOrNot = $("#fileupload").val();
			...
			if (!ExcelExistsOrNot) {
				$scope.ErrorForMultipleUserType = "Please browse Coupon code excel.";
				return;
			}
			$scope.model.saving = true;
			//if (CouponCode.CouponCodeId == 0) {
			...
			$http.post(Comman.CreateEditCouponCodeUrlExcelCoupon(), Input)
				.then(function (response) {
					if (response.status == 200 && response.data.StatusCode == 200) {
						notificationsService.success("Success", response.data.Message);
						$scope.init();
	...
	/*Save excel data for serial no*/
	$scope.SaveCouponCodeExcelSerial = function (CreateCouponCodeFormExcelSerial, CouponCode) {
		$scope.ErrorForAgeRequired = '';
		$scope.ErrorForSubscriptionRequired = '';
		$scope.ErrorForMultipleUserType = '';

		if (CreateCouponCodeFormExcelSerial.$invalid || $scope.EndDateErrorMessage != '') {
			return;
		} else {
			var SelectedUserType = "";
			var SelectedSubscription = "";
			...
			if (!ExcelExistsOrNot) {
				$scope.ErrorForMultipleUserType = "Please browse Serial no. excel.";
				return;
			}
			$scope.model.saving = true;
			//if (CouponCode.CouponCodeId == 0) {
			...
			$http.post(Comman.CreateEditCouponCodeUrlExcelSerial(), Input)
				.then(function (response) {
					if (response.status == 200 && response.data.StatusCode == 200) {
						notificationsService.success("Success", response.data.Message);
						$scope.init();
